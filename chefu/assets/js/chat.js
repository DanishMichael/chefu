...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: "http://localhost/Chefu/Chat?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post("http://localhost/chefu/Chat?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post("http://localhost/chefu/Chat?action=sendchat", {to: chatboxtitle, message: message} , function(data){
...
function startChatSession(){  
	jQuery.ajax({
	  url: "http://localhost/chefu/Chat?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
